<template>
	<el-dialog v-model="visible" title="查看登录日志" :close-on-click-modal="false" draggable width="480px">
...
// import apis
import { $getLoginLogData } from '@/api/bms/log/loginLog'
...
const init = async (loginLogId)=>{
	// get login log data
	await getLoginLogData(loginLogId)
	// set visible
	visible.value = true
}
 ...
 * @param loginLogId
 */
const getLoginLogData = async (loginLogId)=>{
	// get login log data
	let handler = await $getLoginLogData(loginLogId)
	// set login log data
