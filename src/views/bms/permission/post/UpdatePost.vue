<template>
	<el-dialog v-model="visible" title="编辑岗位" :close-on-click-modal="false" draggable width="480px">
		<el-form ref="formRef" :model="post" :rules="rules" label-width="120px" @keyup.enter="updatePost">
			...
			<el-form-item>
				...
				<el-button type="success" @click="updatePost"><sw-icon icon="save"></sw-icon>保存</el-button>
...
// import apis
import { $getPostData, $updatePost } from '@/api/bms/permission/post'
...
// const rules
const rules = ref({
	postCode: [{ label:'岗位编码', valid:'r|l-le200', unique:{ code:'bms.permission.post.post_code.update', args:[post.postId] }, lang:t, validator:validator, trigger:'blur' }],
...
const getPostData = async (postId: number)=>{
	...
	// set post data
	Object.assign(post, handler.data)
}
