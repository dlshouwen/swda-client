<template>
	<el-dialog v-model="visible" title="编辑授权平台" :close-on-click-modal="false" draggable width="480px">
		<el-form ref="formRef" :model="authPlatform" :rules="rules" label-width="120px" @keyup.enter="updateAuthPlatform">
			...
			<el-form-item>
				...
				<el-button type="success" @click="updateAuthPlatform"><sw-icon icon="save"></sw-icon>保存</el-button>
...
// import apis
import { $getAuthPlatformData, $updateAuthPlatform } from '@/api/bms/platform/authPlatform'
...
const getAuthPlatformData = async (authPlatformId: number)=>{
	...
	Object.assign(authPlatform, handler.data)
}
...
const updateAuthPlatform = ()=>{
	...
	formRef.value.validate((valid:boolean)=>{
		...
		// update auth platform
		$updateAuthPlatform(authPlatform).then(()=>{
			// message
