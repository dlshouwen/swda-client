		...
		<div class="panel data-panel">
			...
			<div class="panel-header">
				...
				<div class="panel-title">
					<el-link icon="plus">新增机构</el-link>
				</div>
			</div>
			<!-- panel body -->
				...
				<el-row style="height:100%">
					<el-col :span="8" style="height:100%">
						...
						<sw-form ref="organForm" :model="organ">
							<!-- items -->
							<sw-form-item type="text" prop="organId" v-model="organ.organId" label="机构编号" valid="r|integer" :valid-unique="{code:'organ.id.add', args:[]}"></sw-form-item>
							<sw-form-item type="text" prop="organCode" v-model="organ.organCode" label="机构编码" valid="r|english_number|l-le200" :valid-unique="{code:'organ.code.add', args:[]}"></sw-form-item>
							...
							<!-- operation -->
							<sw-form-item>
								<sw-button type="success" :underline="false" icon="circle-check" @click="addOrgan" permission="bms:system:organ:add">保存 (S)</sw-button>
...
// import requests: get region cascader list, get organ data, add organ
import { $getRegionCascaderList, $getOrganData, $addOrgan } from '@/assets/config/api';
...
const getOrganData = () => {
   	// get organ data
	$getOrganData({organId:-1}).then(handler=>{
		// set organ
...
const addOrgan = () => {
	...
	organForm.value.validate((valid)=>{
		if(valid){
			// confirm
			ElMessageBox.confirm('确定新增机构信息？', '提示').then(()=>{
				// add organ
				$addOrgan(state.organ).then(handler=>{
					// success
...
// hotkey
key('alt+s', ()=>{ addOrgan();return false; });
