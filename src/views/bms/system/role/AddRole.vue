		...
		<div class="panel data-panel">
			...
			<div class="panel-header">
				...
				<div class="panel-title">
					<el-link icon="plus">新增角色</el-link>
				</div>
			</div>
			<!-- panel body -->
			<div class="panel-body">
				...
				<sw-form ref="roleForm" :model="role">
					<!-- items -->
					<sw-form-item type="select" prop="systemId" v-model="role.systemId" label="所属系统" :items="selectSystems" item-key="systemId" item-value="systemName" valid="r"></sw-form-item>
					<sw-form-item type="text" prop="roleId" v-model="role.roleId" label="角色编号" valid="r|integer" :valid-unique="{code:'role.id.add', args:[]}"></sw-form-item>
					<sw-form-item type="text" prop="roleCode" v-model="role.roleCode" label="角色编码" valid="r|english_number|l-le200" :valid-unique="{code:'role.code.add', args:[]}"></sw-form-item>
					...
					<!-- operation -->
					<sw-form-item>
						<sw-button type="success" :underline="false" icon="circle-check" @click="addRole" permission="bms:system:role:add">保存 (S)</sw-button>
...
// import requests: get system select list, get role data, add role
import { $getSystemSelectList, $getRoleData, $addRole } from '@/assets/config/api';
...
const getRoleData = () => {
   	// get role data
	$getRoleData({roleId:-1}).then(handler=>{
		// set role
...
const addRole = () => {
	...
	roleForm.value.validate((valid)=>{
		if(valid){
			// confirm
			ElMessageBox.confirm('确定新增角色信息？', '提示').then(()=>{
				// add role
				$addRole(state.role).then(handler=>{
					// success
...
// hotkey
key('alt+s', ()=>{ addRole();return false; });
