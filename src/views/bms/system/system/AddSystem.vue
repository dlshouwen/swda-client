		...
		<div class="panel data-panel">
			...
			<div class="panel-header">
				...
				<div class="panel-title">
					<el-link icon="plus">新增系统</el-link>
				</div>
			</div>
			<!-- panel body -->
			<div class="panel-body">
				<!-- form -->
				<sw-form ref="systemForm" :model="system" label-width="80px" :show-message="false" :status-icon="true">
					<!-- items -->
					<sw-form-item type="text" prop="systemId" v-model="system.systemId" label="系统编号" valid="r|integer"></sw-form-item>
					<sw-form-item type="text" prop="systemCode" v-model="system.systemCode" label="系统编码" valid="r|english_number|l-le200" :valid-unique="{code:'system.code.add', args:[]}"></sw-form-item>
					...
					<!-- operation -->
					<sw-form-item>
						<sw-button type="success" :underline="false" icon="circle-check" @click="addSystem" permission="bms:system:system:add">保存 (S)</sw-button>
...
// import requests: get system data, add system
import { $getSystemData, $addSystem } from '@/assets/config/api';
...
const getSystemData = () => {
   	// get system data
	$getSystemData({systemId:-1}).then(handler=>{
		// set system
...
const addSystem = () => {
	...
	systemForm.value.validate((valid)=>{
		if(valid){
			// confirm
			ElMessageBox.confirm('确定新增系统信息？', '提示').then(()=>{
				// add system
				$addSystem(state.system).then(handler=>{
					// success
...
// hotkey
key('alt+s', ()=>{ addSystem();return false; });
