		...
		<el-row style="height:100%">
				...
				<div class="panel data-panel">
					<div class="panel-header">
						<div class="panel-title">
							<el-link icon="edit">编辑任务</el-link>
						</div>
					</div>
					<div class="panel-body">
						...
						<sw-form ref="taskForm" :model="task" label-width="64px">
							<!-- items -->
							<sw-form-item type="text" prop="taskId" v-model="task.taskId" label="任务编号" disabled></sw-form-item>
							<sw-form-item type="text" prop="taskCode" v-model="task.taskCode" label="任务编码" valid="r|english_number|l-le200" :valid-unique="{code:'task.code.update', args:[task.taskId]}"></sw-form-item>
							...
							<!-- operation -->
							<sw-form-item>
								<sw-button type="success" :underline="false" icon="circle-check" @click="updateTask" permission="bms:system:task:update">保存 (S)</sw-button>
			...
			<el-col :span="12" style="height:100%">
				...
				<div class="panel data-panel" style="margin-left:0;">
					...
					<div class="panel-body">
						<sw-grid ref="userGrid" id="user_list" title="用户列表" url="/bms/system/task/user/list" row-key="userId" :params="{taskId: taskId}" :height="200"
				...
				<div class="panel data-panel" style="margin-left:0;">
					...
					<div class="panel-body">
						<sw-grid ref="roleGrid" id="role_list" title="角色列表" url="/bms/system/task/role/list" row-key="roleId" :params="{taskId: taskId}" :height="180" 
							:toolbar-label="false" layout="prev, pager, next, total"
...
// import requests: get permission tree, get task role list, get task user list, get task data, update task
import { $getPermissionTreeList, $getTaskRoleList, $getTaskUserList, $getTaskData, $updateTask } from '@/assets/config/api';
...
const getPermissionTreeList = () => {
	...
	$getPermissionTreeList({systemId:'', permissionType:'1'}).then(handler=>{
		...
		// set permission tree checked
		permissionTree.value.setChecked('p'+state.task.permissionId, true);
	});
}
...
const getTaskData = () => {
   	// get task data
	$getTaskData({taskId:props.taskId}).then(handler=>{
		// set task
...
const updateTask = () => {
	...
	taskForm.value.validate((valid)=>{
		if(valid){
			// confirm
			ElMessageBox.confirm('确定编辑任务信息？', '提示').then(()=>{
				// set role ids
				let roleIds = [];
				...
				// edit task
				$updateTask(state.task).then(handler=>{
					// success
...
// watch
watch(()=>props.taskId, ()=>{ reset() }, {deep: true}) 
...
// hotkey
key('alt+s', ()=>{ updateTask();return false; });
