<template>
	<el-dialog v-model="visible" title="编辑任务" :close-on-click-modal="false" draggable width="480px">
		<el-form ref="formRef" :model="task" :rules="rules" label-width="120px" @keyup.enter="updateTask">
			...
			<el-form-item>
				...
				<el-button type="success" @click="updateTask"><sw-icon icon="save"></sw-icon>保存</el-button>
...
// import apis
import { $getTaskData, $updateTask } from '@/api/bms/system/task'
...
// const rules
const rules = ref({
	taskCode: [{ label:'任务编码', valid:'r|l-le200', unique: { code:'bms.system.task.task_code.update', args:[task.taskId] }, lang:t, validator:validator, trigger:'blur' }],
...
const getTaskData = async (taskId)=>{
	...
	// set task data
	Object.assign(task, handler.data)
}
