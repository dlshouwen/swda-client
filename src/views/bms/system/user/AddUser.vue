		...
		<div class="panel data-panel">
			...
			<div class="panel-header">
				...
				<div class="panel-title">
					<el-link icon="plus">新增用户</el-link>
				</div>
			</div>
			<!-- panel body -->
			<div class="panel-body">
				...
				<sw-form ref="userForm" :model="user">
					...
					<sw-form-item type="text" prop="userId" v-model="user.userId" label="用户编号" valid="r|integer" :valid-unique="{code:'user.id.add', args:[]}"></sw-form-item>
					<sw-form-item type="text" prop="userCode" v-model="user.userCode" label="用户编码" valid="r|english_number|l-le200" :valid-unique="{code:'user.code.add', args:[]}"></sw-form-item>
					...
					<!-- operation -->
					<sw-form-item>
						<sw-button type="success" :underline="false" icon="circle-check" @click="addUser" permission="bms:system:user:add">保存 (S)</sw-button>
...
// import requests: get select organ list, get user data, add user
import { $getSelectOrganList, $getUserData, $addUser } from '@/assets/config/api';
...
const getUserData = () => {
 	// get user data
	$getUserData({userId:null}).then(handler=>{
		// set user
...
const addUser = () => {
	...
	userForm.value.validate((valid)=>{
		if(valid){
			// confirm
			ElMessageBox.confirm('确定新增用户信息？', '提示').then(()=>{
				// add user
				$addUser(state.user).then(handler=>{
					// success
...
// hotkey
key('alt+s', ()=>{ addUser();return false; });
